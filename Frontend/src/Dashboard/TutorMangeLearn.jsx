...
export default function TutorManageLearn() {
  ...
  const [form, setForm] = useState({
    ...
    image: "",
    modules: [
      { title: "", description: "", content: "", codeExample: "", image: "" },
    ],
  });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`https://tb-back-fyvj.onrender.com/api/learn/${id}`, {
        method: "DELETE",
      });
      fetchLearnData();
    } catch (err) {
  ...
  return (
    <div className="tutor-learn-container">
      <h1 className="tutor-learn-title">Tutor Manage Learn Modules</h1>

      <form onSubmit={handleSubmit} className="tutor-learn-form">
      ...
      <div className="tutor-learn-list">
        <h2>All Courses</h2>
        <div className="learn-list-grid">
          {learns.map((item) => (
            <div key={item._id} className="learn-card-tutor">
              <img
                src={item.image}
                alt={item.language}
                className="learn-img-tutor"
              />
              <h3>{item.language}</h3>
