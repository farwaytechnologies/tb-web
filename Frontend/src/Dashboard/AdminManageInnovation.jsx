...
export default function AdminManageInnovation() {
  ...
  const fetchInnovations = async () => {
    try {
      const res = await fetch('VITE_API_URL/api/innovations');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editId
        ? `VITE_API_URL/api/innovations/${editId}`
        : 'VITE_API_URL/api/innovations';
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this innovation?')) {
      try {
        const res = await fetch(`VITE_API_URL/api/innovations/${id}`, {
          method: 'DELETE',
        });
